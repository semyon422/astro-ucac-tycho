...
-- поделим на N чуть позже

-- если посчитать сначала в градусах, а потом в миллисекундах,
-- то значения d_alpha_6, приведённые к миллисекундам, будут совпадать
-- примерно в первых 10-12 десятичных знаках
-- (напр. -934.93937796568 и -934.93937796331) для первой пары
-- даже такая немалая точность может привести к большой разнице в ответе
-- покажем это, посчитав погрешность ksi_alpha,
-- считая, что абс. погрешность d_alpha_6 равна |d_alpha_6 * 10^-10| градусов

-- абсолютная погрешность суммы - сумма погрешностей

local abs_ksi = 0
for i = 1, N do
	local pair = _pairs[i]
	abs_ksi = abs_ksi + math.abs(pair.d_alpha_6 * 1e-10)
end
print("abs_ksi", abs_ksi)  -- 5.1819937614364e-09

-- относительная погрешность частного равна сумме относительных погрешностей делителя и делимого
-- отн. погрешность N равна 0, то есть отн. погрешность ksi_alpha после деления на N не изменится
-- посчитаем относительную погрешность ksi_alpha,
-- поделим абсолютную погрешность на само значение, возмём под модуль:

local rel_ksi_alpha = math.abs(abs_ksi / ksi_alpha)
print("rel_ksi_alpha", rel_ksi_alpha)  -- 1695017.0929072

ksi_alpha = ksi_alpha / N
ksi_delta = ksi_delta / N

-- деление на N не меняет относительную погрешность,
-- поэтому считаем абс. погрешность после деления чтоб сравнить с ответом
print("abs_ksi_alpha", ksi_alpha * rel_ksi_alpha)  -- 2.5757330864479e-15

-- получившееся значение абсолютной погрешности
-- на 6 порядков больше чем ksi_alpha
-- это говорит о том, что в алгоритме вычисления ksi_alpha
-- очень сильно теряется точность, и итоговое значение может отличаться на порядки

print("N:", N)  -- 2011852
print("ksi_alpha:", ksi_alpha)  -- 1.5195912166467e-21
print("ksi_delta:", ksi_delta)  -- 7.1081076722473e-21
